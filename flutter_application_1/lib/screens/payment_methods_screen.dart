...
class _PaymentMethodsScreenState extends State<PaymentMethodsScreen> {
  ...
  int get _defaultIdx => _cards.indexWhere((c) => c['isDefault'] == true);
  ...
  Future<void> _loadCards() async {
    final result = await ApiService.instance.getCards();
    if (mounted) {
      setState(() {
        _loadingCards = false;
        _cards = result['success']
            ? (result['data'] as List).map((c) => Map<String, dynamic>.from(c)).toList()
            : [];
      });
    }
  }
  ...
  Future<void> _loadTransactions() async {
    ...
    if (mounted) {
      setState(() {
        _loadingTx = false;
        if (result['success']) {
          _transactions = (result['data'] as List).map((t) {
            final amount = (t['amount'] as num).toDouble();
            return {
              'label':  t['label'] as String,
              'date':   _formatDate(t['createdAt'] as String?),
              'amount': amount >= 0 ? '+${amount.toStringAsFixed(1)}' : '${amount.toStringAsFixed(1)}',
  ...
  String _formatDate(String? iso) {
    ...
    try {
      ...
      return '${d.day} Apr ${d.hour}:${d.minute.toString().padLeft(2, '0')}';
    } catch (_) { return 'Now'; }
  }
  ...
  void _topUp() {
    ...
    int selCard   = _defaultIdx;
    double? parsed;
    final quick   = [10.0, 25.0, 50.0, 100.0, 200.0];
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Row(children: [
              ...
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('Balance: ${UserSession.instance.balance.toStringAsFixed(2)} NIS', style: kSub(12)),
              ]),
            ]),
            const SizedBox(height: 20),
            TextField(
              ...
              onChanged: (v) => set(() => parsed = double.tryParse(v)),
            ...
            Row(children: quick.map((a) {
              final sel = parsed == a;
              return Expanded(child: GestureDetector(
                onTap: () { amtCtrl.text = a.toStringAsFixed(0); set(() => parsed = a); },
                child: Container(
                  ...
                  child: Center(child: Text(a.toStringAsFixed(0),
                      style: TextStyle(color: sel ? kGreen : cSub, fontSize: 12, fontWeight: FontWeight.w700))))));
            }).toList()),
            ...
            SizedBox(width: double.infinity, height: 52,
              child: ElevatedButton(
                onPressed: (parsed != null && parsed! > 0)
                    ? () async {
                        Navigator.pop(ctx);
                        final result = await ApiService.instance.topUp(parsed!);
                        if (mounted) {
                          _snack(result['success']
                              ? 'NIS ${parsed!.toStringAsFixed(2)} added! ✅'
                              : result['message']);
                          if (result['success']) _loadTransactions();
                        }
                      }
                    : null,
                ...
                child: Text(
                  parsed != null && parsed! > 0 ? 'Top Up  NIS ${parsed!.toStringAsFixed(2)}' : 'Enter Amount',
  ...
  void _transfer() async {
    final result = await showModalBottomSheet<Map<String, dynamic>>(
      context: context, isScrollControlled: true, backgroundColor: cCard,
      shape: const RoundedRectangleBorder(borderRadius: BorderRadius.vertical(top: Radius.circular(24))),
      builder: (_) => _TransferSheet(balance: UserSession.instance.balance));

    if (result != null && mounted) {
      final amount   = result['amount'] as double;
      final typeName = result['typeName'] as String;
      final res = await ApiService.instance.transfer(
          amount: amount, destination: result['dest'] ?? '', typeName: typeName);
      if (mounted) {
        _snack(res['success'] ? 'NIS ${amount.toStringAsFixed(2)} transferred! ✅' : res['message']);
        if (res['success']) _loadTransactions();
      }
    }
  }
  ...
  void _addCard() async {
    ...
    if (result != null && mounted) {
      final res = await ApiService.instance.addCard(result);
      if (mounted) {
        _snack(res['success'] ? 'Card added! ✅' : res['message']);
        if (res['success']) _loadCards();
      }
    }
  }
  ...
  void _cardOptions(int idx) {
    ...
    final cardId = card['_id'] as String? ?? '';
      ...
      builder: (_) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(width: 36, height: 4,
              decoration: BoxDecoration(color: cBorder, borderRadius: BorderRadius.circular(2))),
          const SizedBox(height: 20),
          Text(card['number'] as String, style: kTitle(16)),
          const SizedBox(height: 20),
          if (!isDefault) _optTile(Icons.star_outline, kGreen, 'Set as Default', () async {
            Navigator.pop(context);
            final res = await ApiService.instance.setDefaultCard(cardId);
            if (mounted) {
              _snack(res['success'] ? 'Default updated ✅' : res['message']);
              if (res['success']) _loadCards();
            }
          }),
          _optTile(Icons.delete_outline, Colors.redAccent, 'Remove Card', () async {
            Navigator.pop(context);
            final res = await ApiService.instance.deleteCard(cardId);
            if (mounted) {
              _snack(res['success'] ? 'Card removed' : res['message']);
              if (res['success']) _loadCards();
            }
          }),
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('NIS ${balance.toStringAsFixed(2)}',
            ...
            _loadingTx
                ...
                : _transactions.isEmpty
                    ? Container(padding: const EdgeInsets.all(20), decoration: kCardDeco(),
                        child: Center(child: Text('No transactions yet', style: kSub(13))))
...
class _TransferSheetState extends State<_TransferSheet> {
  ...
  Widget _step1() => Column(mainAxisSize: MainAxisSize.min, children: [
    _typeOpt(Icons.person_outline,   kGreen,               'ChargeGuard User', 'Transfer by phone', 'user',   'ChargeGuard User'),
    const SizedBox(height: 10),
    _typeOpt(Icons.credit_card_outlined, Colors.blueAccent, 'Bank Card',       'Via IBAN',          'card',   'Bank Card'),
    const SizedBox(height: 10),
    _typeOpt(Icons.account_balance_wallet_outlined, const Color(0xFF003087), 'PayPal', 'By email', 'paypal', 'PayPal'),
  ]);

  Widget _typeOpt(IconData icon, Color color, String title, String sub, String type, String name) =>
    GestureDetector(
  ...
  Widget _step2() {
    final labels = {
      'user':   {'label': 'Phone Number', 'hint': '+970 5X XXX XXXX'},
      'card':   {'label': 'IBAN Number',  'hint': 'PS00 XXXX XXXX XXXX'},
      'paypal': {'label': 'PayPal Email', 'hint': 'example@paypal.com'},
    };
    final info = labels[_type]!;
    return Column(mainAxisSize: MainAxisSize.min, children: [
      GestureDetector(onTap: () => setState(() => _step = 1),
        child: Row(children: [Icon(Icons.arrow_back_ios_new, size: 14, color: cSub), const SizedBox(width: 4),
      ...
      TextField(controller: _amtCtrl,
        ...
        style: TextStyle(color: cTitle, fontSize: 22, fontWeight: FontWeight.w800), textAlign: TextAlign.center,
        decoration: InputDecoration(hintText: '0.00', hintStyle: TextStyle(color: cSub2, fontSize: 22),
          prefixText: 'NIS  ', prefixStyle: const TextStyle(color: kGreen, fontSize: 18, fontWeight: FontWeight.w700),
          filled: true, fillColor: cBg,
          border: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide(color: cBorder)),
          enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide(color: cBorder)),
          focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: const BorderSide(color: kGreen, width: 2)))),
      ...
      const SizedBox(height: 14),
      TextField(controller: _destCtrl, onChanged: (_) => setState(() {}),
        style: TextStyle(color: cTitle, fontSize: 15),
        decoration: InputDecoration(labelText: info['label'], labelStyle: TextStyle(color: cSub),
          hintText: info['hint'], hintStyle: TextStyle(color: cSub2, fontSize: 12),
          filled: true, fillColor: cBg,
          border: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide(color: cBorder)),
          enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(14), borderSide: BorderSide(color: cBorder)),
          ...
          onPressed: (_parsed != null && _parsed! > 0 && _destCtrl.text.trim().isNotEmpty)
              ? () {
                  if (_parsed! > widget.balance) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: const Text('Insufficient balance!'),
              ...
              : null,
          style: ElevatedButton.styleFrom(backgroundColor: kGreen, disabledBackgroundColor: kGreen.withOpacity(0.2),
              foregroundColor: Colors.black, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
          child: const Text('Review Transfer', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800)))),
    ]);
  }

  Widget _step3() => Column(mainAxisSize: MainAxisSize.min, children: [
    GestureDetector(onTap: () => setState(() => _step = 2),
      child: Row(children: [Icon(Icons.arrow_back_ios_new, size: 14, color: cSub), const SizedBox(width: 4),
        Text('Back', style: TextStyle(color: cSub, fontSize: 13))])),
    const SizedBox(height: 16),
    Container(width: double.infinity, padding: const EdgeInsets.all(20),
      ...
      child: Column(children: [
        ...
        const SizedBox(height: 16),
        _row('To', _typeName), _row('Destination', _destCtrl.text),
        _row('After', 'NIS ${(widget.balance - _parsed!).toStringAsFixed(2)}'),
      ])),
    const SizedBox(height: 14),
    Container(padding: const EdgeInsets.all(11),
      ...
      child: Row(children: [
        const Icon(Icons.warning_amber_outlined, color: Colors.orange, size: 16), const SizedBox(width: 8),
        Expanded(child: Text('Transfers cannot be reversed.', style: TextStyle(color: cSub, fontSize: 11))),
      ])),
    const SizedBox(height: 16),
    SizedBox(width: double.infinity, height: 52,
      child: ElevatedButton(
        onPressed: () => Navigator.pop(context, {
          'amount': _parsed, 'typeName': _typeName, 'dest': _destCtrl.text}),
        style: ElevatedButton.styleFrom(backgroundColor: kGreen, foregroundColor: Colors.black,
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
        child: const Text('Confirm Transfer', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800)))),
  ]);

  Widget _row(String l, String v) => Padding(padding: const EdgeInsets.symmetric(vertical: 5),
    child: Row(children: [Text('$l:', style: kSub(13)), const Spacer(),
      Text(v, style: TextStyle(color: cTitle, fontSize: 13, fontWeight: FontWeight.w700))]));
}
...
class _AddCardSheetState extends State<_AddCardSheet> {
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 20),
        Row(children: ['Visa', 'Mastercard', 'PayPal'].map((t) {
        ...
        const SizedBox(height: 14),
        _f(_numberCtrl, 'Card Number', Icons.credit_card_outlined, type: TextInputType.number, hint: '0000 0000 0000 0000'),
        const SizedBox(height: 12),
        _f(_holderCtrl, 'Holder Name', Icons.person_outline, hint: 'Name on card'),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(child: _f(_expiryCtrl, 'Expiry', Icons.date_range_outlined, hint: 'MM/YY', type: TextInputType.number)),
          const SizedBox(width: 12),
          Expanded(child: TextField(controller: _cvvCtrl, obscureText: _obscureCvv,
            keyboardType: TextInputType.number, maxLength: 3,
            style: TextStyle(color: cTitle, fontSize: 15),
            decoration: InputDecoration(labelText: 'CVV', labelStyle: TextStyle(color: cSub), counterText: '',
              prefixIcon: Icon(Icons.lock_outline, color: cSub2, size: 20),
              suffixIcon: IconButton(icon: Icon(_obscureCvv ? Icons.visibility_outlined : Icons.visibility_off_outlined, color: cSub2, size: 18),
        ...
        SizedBox(width: double.infinity, height: 52,
          child: ElevatedButton(
            onPressed: () {
              if ([_numberCtrl, _holderCtrl, _expiryCtrl, _cvvCtrl].any((c) => c.text.trim().isEmpty)) return;
              ...
              Navigator.pop(context, {
                ...
                'color1': _cardType == 'Visa' ? 0xFF1A1F71 : _cardType == 'Mastercard' ? 0xFF1A1A1A : 0xFF003087,
                'color2': _cardType == 'Visa' ? 0xFF2563EB : _cardType == 'Mastercard' ? 0xFF333333 : 0xFF009CDE,
                'icon':   _cardType == 'Visa' ? 'VISA' : _cardType == 'Mastercard' ? 'MC' : 'PP',
              });
            },
            style: ElevatedButton.styleFrom(backgroundColor: kGreen, foregroundColor: Colors.black,
                shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
  ...
  Widget _f(TextEditingController ctrl, String label, IconData icon, {TextInputType? type, String? hint}) =>
    TextField(controller: ctrl, keyboardType: type, style: TextStyle(color: cTitle, fontSize: 15),
